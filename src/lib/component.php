...
class component
{
    ...
    private $getComponentAccessTokenUrl = "https://api.weixin.qq.com/cgi-bin/component/api_component_token";                                        //获取令牌
    private $getPreAuthCodeUrl = "https://api.weixin.qq.com/cgi-bin/component/api_create_preauthcode?component_access_token=%s";                    //预授权码
    private $getComponentLoginPageUrl = "https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid=%s&pre_auth_code=%s&redirect_uri=%s";  //授权页
    private $getApiQueryAuthUrl = "https://api.weixin.qq.com/cgi-bin/component/api_query_auth?component_access_token=%s";                           //使用授权码获取授权信息
    private $getApiAuthorizerTokenUrl = "https://api.weixin.qq.com/cgi-bin/component/api_authorizer_token?component_access_token=%s";               //获取/刷新接口调用令牌
    private $getApiGetAuthorizerInfoUrl = "https://api.weixin.qq.com/cgi-bin/component/api_get_authorizer_info?component_access_token=%s";          //获取授权方的帐号基本信息
    private $getApiGetAuthorizerOptionUrl = "https://api.weixin.qq.com/cgi-bin/component/api_get_authorizer_option?component_access_token=%s";      //获取授权方选项信息
    private $getApiSetAuthorizerOptionUrl = "https://api.weixin.qq.com/cgi-bin/component/api_set_authorizer_option?component_access_token=%s";      //设置授权方选项信息

    private $getAuthorizerListUrl = "https://api.weixin.qq.com/cgi-bin/component/api_get_authorizer_list?component_access_token=%s";                //拉取所有已授权的帐号信息 post
    private $openCreateUrl = "https://api.weixin.qq.com/cgi-bin/open/create?access_token=%s";                                                       //创建开放平台帐号并绑定公众号/小程序 post
    private $openBindUrl = "https://api.weixin.qq.com/cgi-bin/open/bind?access_token=%s";                                                           //将公众号/小程序绑定到开放平台帐号下 post
    private $openUnbindUrl = "https://api.weixin.qq.com/cgi-bin/open/unbind?access_token=%s";                                                       //将公众号/小程序从开放平台帐号下解绑 post
    private $openGetUrl = "https://api.weixin.qq.com/cgi-bin/open/get?access_token=%s";                                                             //获取公众号/小程序所绑定的开放平台帐号 post
     ...
     * @return core\mix
     */
    public function apiGetAuthorizerList($component_appid, $component_access_token, $offset, $count)
    {
        $url = sprintf($this->getAuthorizerListUrl, $component_access_token);
        $postData = array(
            "component_appid" => $component_appid,
            "offset" => $offset,
            "count" => $count
    ...
    {
        $url = sprintf($this->openCreateUrl, $access_token);
        $postData = [
            'appid' => $appid
        ];
        return http::post($url, [], json_encode($postData));
    ...
    {
        $url = sprintf($this->openBindUrl, $access_token);
        $postData = [
            'appid' => $appid,
    ...
    {
        $url = sprintf($this->openUnbindUrl, $access_token);
        $postData = [
            'appid' => $appid,
            'open_appid' => $open_appid
        ];
    ...
    {
        $url = sprintf($this->openGetUrl, $access_token);
        $postData = [
            'appid' => $appid,
        ];
        return http::post($url, [], json_encode($postData));
